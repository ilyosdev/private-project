...
const _SS = e => document.querySelectorAll(e);
...
const loading = _S('.overlay-loading');
const alertBox = _S('.alert-box');
const confirmBox = _S('.confirm-box');
const promptBox = _S('.prompt-box');
...
// ========== DECORATORS =============
const alert = (str = '') => {
    _S('.alert-message').innerHTML = str.trim();
    alertBox.flex();
}

const confirm = (str = '', cb) => {
    _S('.confirm-message').innerHTML = str.trim();
    confirmBox.flex();
...
const prompt = (str = '', cb) => {
    let label = _S('.prompt-message').children[0];
    let input = _S('.prompt-message').children[2];

    label.innerHTML = str.trim();
    promptBox.flex();

    let btnYes = _S('.prompt-actions').children[1];
    let btnNo = _S('.prompt-actions').children[0];
...
_S('.btn-add-chapter').on('click', (event) => {
